...
public class ParkingService {
		...
		try {
			...
			if (parkingSpot != null && parkingSpot.getId() > 0) {
				...
				if (ticketDAO.noDoubleRegNumber(vehicleRegNumber) == true) {
					...
					System.out.println(
							"Error, a vehicle with the same registration number is already in the parking. Process Cancelled");
			}
		} catch (Exception e) {
	...
	public void processExitingVehicle() {
		try {
			...
			fareCalculatorService.calculateFare(ticket, ticketDAO.availableReduction5Percent(ticket));
			...
			throw new RuntimeException("Unable to process exiting vehicle");
		}
	}
